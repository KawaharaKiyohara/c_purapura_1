...
bool BinarySearch(int value[], int numValues, int target)
{
	...
	while (left <= right) {
		mid = (left + right) / 2; //leftとrightの中間を起点とする。
		if (value[mid] == target) {
			//見つかった。
			return true;
		}
		else if (value[mid] < target) { //value[mid]が目標値よりも小さい、ということは
										//midより左の要素は、すべてvalue[mid]より小さいので、もう検索しなくてよい。
			left = mid + 1;
		}
		else {							//value[mid]が目標値よりも大きい、ということは
										//midより右の要素は、すべてvalue[mid]より大きいので、もう検索しなくてよい。
...
int main()
{
	...
	//   27行目～の34行目までの100回繰り返されるループでは、毎回、0～1999999のランダムな整数を生成して、   
