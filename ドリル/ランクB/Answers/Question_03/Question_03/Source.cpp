...
int main()
{
	...
	for (int i = 0; i < 100; i++) {
		if (characterAabb.vMax.x > positions[i].x		//点のx座標が箱の最大x座標より小さい。
			&& characterAabb.vMin.x < positions[i].x	//点のx座標が箱の最小x座標より大きい。
			&& characterAabb.vMax.y > positions[i].y	//点のy座標が箱の最大y座標より小さい。
			&& characterAabb.vMin.y < positions[i].y	//点のy座標が箱の最小y座標より大きい。
		) {
			//衝突してるね。
			count++;
		}
