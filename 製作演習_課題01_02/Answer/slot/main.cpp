...
int main()
{
	...
	while (true) 
	{
		...
		//ここから課題のリールを止めるプログラムと、結果を表示するプログラムを書く！
		if (kbcIsTriggerA() == 1) 
		{
			if (kaitenReel_1_flag == 1) 
			{
			...
			else if (kaitenReel_2_flag == 1) 
			{
				//2番目のリールを止める。
				kaitenReel_2_flag = 0;
			}
			else if (kaitenReel_3_flag == 1) 
			{
				//3番目のリールを止める。
				kaitenReel_3_flag = 0;
			}
		...
		//結果を表示
		if ( kaitenReel_1_flag == 0
			&& kaitenReel_2_flag == 0
			&& kaitenReel_3_flag == 0 ) 
		{
			if (reel_1 == 7 && reel_2 == 7 && reel_3 == 7) 
			{
				kbcDrawMojiRetu(2, 7, "大当たり！！！");
			}else if (reel_1 == reel_2 && reel_2 == reel_3) 
			{
				kbcDrawMojiRetu(2, 7, "当たり！！！");
			}
			else 
			{
				kbcDrawMojiRetu(4, 7, "残念！！！");
			}
		}
		
		// ここまでの間にゲームの更新プログラムを書いていってね//////////////////////
