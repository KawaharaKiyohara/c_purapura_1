...
void CPathFinding::FindRoot(std::vector<Vector2>& root,  Vector2 startPos, Vector2 targetPos)
{
	root.clear();
	//コストを初期化
	for (int y = 0; y < 16; y++) {
	...
	//開始ノードを取得する。
	SNode* startNode = &m_nodes[startPos.y][startPos.x];
	...
	//ルートを作る。
	SNode* node = &m_nodes[targetPos.y][targetPos.x];
	
	while (node != startNode) {
		root.push_back(node->pos);
